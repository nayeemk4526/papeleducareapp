...
const EnrollmentDialog = ({ open, onOpenChange, courseId }: EnrollmentDialogProps) => {
  ...
  const [selectedCourseId, setSelectedCourseId] = useState<string | null>(courseId ? String(courseId) : null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await enrollUser.mutateAsync({ userId: selectedUserId, courseId: selectedCourseId });
    onOpenChange(false);
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <Input value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="নাম, ইমেইল বা ফোন দিয়ে খুঁজুন..." className="pl-10" />
            </div>
          </div>
          {searchTerm && (
            <div className="max-h-48 overflow-y-auto border border-border rounded-lg">
              {filteredUsers.length > 0 ? filteredUsers.map((user) => (
                <div key={user.id} onClick={() => setSelectedUserId(user.user_id)} className={`p-3 cursor-pointer hover:bg-muted transition-colors border-b border-border last:border-0 ${selectedUserId === user.user_id ? "bg-primary/10" : ""}`}>
          ...
          {selectedUserId && (
            <div className="p-3 bg-primary/5 border border-primary/20 rounded-lg">
              <p className="text-sm text-primary font-medium">নির্বাচিত: {users?.find(u => u.user_id === selectedUserId)?.full_name}</p>
            </div>
          )}
          <div className="flex justify-end gap-3 pt-4">
            <Button type="button" variant="outline" onClick={() => onOpenChange(false)}>বাতিল</Button>
            <Button type="submit" disabled={!selectedUserId || !selectedCourseId || enrollUser.isPending} className="gradient-primary">
